...
class Bbvi_bayes_lm:
    ...
    def mc_logprob(data: Dict,
        ...
        lpred = Bbvi_bayes_lm.calc_lpred(data, samples)

        # response dist
        dist_response = tfjd.Normal(loc=lpred, scale=samples["sigma"])

        # calculate the log-likelihood
        log_lik = Bbvi_bayes_lm.loglik(dist_response, data["response"])

        # calculate the scaled log-likelihood
        scaled_log_lik = Bbvi_bayes_lm.calc_scaled_loglik(log_lik,
                                                          num_obs)
        ...
        sigma_a = hyperparams["sigma"]["sigma_a"]
        sigma_b = hyperparams["sigma"]["sigma_b"]
        dist_sigma = tfjd.InverseGamma(concentration=sigma_a, scale=sigma_b)

        log_prior_beta = Bbvi_bayes_lm.logprior(dist_beta, samples["beta"])
        log_prior_sigma = Bbvi_bayes_lm.logprior(dist_sigma, samples["sigma"])
        # print("Log-prior-beta:", log_prior_beta.shape)
        # print("log-prior.sigma:", log_prior_sigma.shape)

        log_prior = jnp.sum(log_prior_beta, axis=-1) + jnp.sum(log_prior_sigma, axis=-1)
    ...
    def lower_bound(variational_params: Dict,
        ...
        for kw in variational_params.keys():
            ...
            if kw == "beta":
                ...
                jac_adjust = batched_jac_determinant(log_transform, samples[kw])
        ...
        mc_log_prob = Bbvi_bayes_lm.mc_logprob(data, hyperparams, samples, num_obs)
    ...
    def run_bbvi(self,
        ...
        def bbvi_body(bbvi_state, data, key):
            ...
            neg_elbo, grads = jax.value_and_grad(Bbvi_bayes_lm.lower_bound)(bbvi_state.params,
                                                                            data,
