...
![Kernel density plots of the posterior means for location parameters of $M_{1}$, true parameters given by $\betavec = [3.0, 0.2, -0.5]^{\mathrm{T}}$.](assets/plots/plot8.pdf){width=80% fig-scap="Kernel density plots for location parameters of $M_{1}$." #fig-kdeplot-loc}

![Kernel density plots of the posterior means for scale parameter of $M_{1}$, true parameter given by $\sigma=1.0$](assets/plots/plot9.pdf){width=80% fig-scap="Kernel density plots for scale parameter of $M_{1}$." #fig-kdeplot-scale}
...
![Kernel density plots of the posterior means for logit parameters of $M_{2}$, true parameters given by $\betavec = [1.0, 2.0]^{\mathrm{T}}$.](assets/plots/plot10.pdf){width=80% fig-scap="Kernel density plots for logit parameters of $M_{2}$." #fig-kdeplot-logit}
...
which we now denote as data generating process (DGP). For the obervation sample size we use $n_{\text{obs}}=1000$. We slightly heightened the frequency of the $\sin$ function to augment the complexity of estimating the function. The target here is to visually asses the fit of the function and compare the posterior distributions of MCMC and BBVI. For estimation of the posterior distribution with BBVI we again use `tigerpy` and for MCMC we use `liesel` [@Riebl2022].

::: {layout="[[50,-5,50]]"}

![Plot of the DGP, mean visualized by the solid line and 95% confidence band by the dashed line.](assets/plots/plot11.pdf){#fig-sim2-dgp}

![Plot of the DGP and the estimated smooth functions of BBVI and MCMC using the posterior mean.](assets/plots/plot12.pdf){#fig-sim2-post-means}

:::

![](assets/plots/plot13.pdf){width=80% #fig-sim2-fixed}

![](assets/plots/plot14.pdf){width=80% #fig-sim2-smooth}

![](assets/plots/plot15.pdf){width=80% #fig-sim2-tau2}

![](assets/plots/plot16.pdf){width=80% #fig-sim2-sig}
...
In this study we also have a target. 

## MSE study

Finally we will compare MSE for different complex function for BBVI and MCMC in reference to @Lang2004.