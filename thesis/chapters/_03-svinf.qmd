...
Throughout the optimization process, we iteratively refine the configuration of variational parameters towards the local optimum. Additionally, we compute the value of the ELBO using the validation set. The optimization runs until we either reach a pre-defined maximum number of epochs $E$ or we stop by triggering some early-stopping criterion. Stopping through the early-stopping criterion indicates that we reached some form of plateau in the ELBO of the validation data. We use $|\text{ELBO}(\hat{\phivec}^{t})_{\mathcal{D}_{\text{val}}} - \text{ELBO}(\hat{\phivec}^{t-200})_{\mathcal{D}_{\text{val}}}| < \epsilon$ as the early stopping criteron, $\epsilon$ being some arbitrary small number. The full BBVI algorithm is roughly outlined in Algorithm \ref{algo}.
...
\begin{algorithm}[H]
    ...
    \Require{Learning rate $\alpha$; num. epochs: $E$; mini-batch size $M$; stopping threshold $\epsilon$; num. var. samples $S$, share train $w$}
    ...
    \For{$e = 1$  \KwTo $E$}{
        ...
        calculate approx. $\text{ELBO}(\hat{\phivec}^{t})_{\mathcal{D}_{\text{val}}}$ \;
        $t = t + 1$ \;
        }
        \eIf{$t > 200$}{
...
Furthermore, we maintain a record of the maximum ELBO attained, denoted as $\text{ELBO}(\hat{\phivec}){\mathcal{D}{\text{val}}}$, along with its corresponding optimal parameter configuration $\hat{\phivec}$. These represent the solutions derived from our optimization problem.

## ELBO convergence {#sec-elbo-conv}

Throughout the process of SGD optimization, we track the convergence of the ELBO.
Exemplary ELBO traces, illustrating the impact of varying optimization parameters on a Bayesian linear regression model, are visualized in @fig-elbo-trace1 through @fig-elbo-trace4. Because the ELBO constitutes a non-convex objective, optimization converges towards a local optimum which is contingent upon its initialization. Moreover as we work with Monte Carlo integration and mini-batches we additionally introduce stochasticity into the optimization.

@fig-elbo-trace1 shows that we usually start with some low value of the ELBO, dependent on the initialization. After that we can observe a steep increase, in which we improve the ELBO fast due to taking large steps in the optimization. This fast convergence tapers off before epoch 50 for all traces. In the following, we are only taking smaller optimization steps because we are already close to the local optimum. The figure also highlights that the optimum is contingent on the initialization. A more effective initialization leads to faster convergence and a superior local optimum.

::: {layout="[[50,-5,50], [50,-5,50]]"}

![ELBO traces for 3 different SGD runs, using different initializations and seeds. We use a batch size of 128 and 64 samples from the variational distribution, with a learning rate of 1e-2.](assets/plots/plot2.pdf){fig-scap="ELBO traces for different init. and seeds." #fig-elbo-trace1}
...
![ELBO traces for 3 different SGD runs, using different variational sample sizes. Using batch VI with a learning rate of 1e-2 and the same seed.](assets/plots/plot4.pdf){fig-sub-scap="ELBO traces for different variational sample sizes." #fig-elbo-trace3}

![ELBO traces for 3 different SGD runs, using different batch sizes. Using a variational sample size of 64 with a learning rate of 1e-2 and the same seed.](assets/plots/plot5.pdf){fig-scap="ELBO traces for different batch sizes." #fig-elbo-trace4}
...
The choice of the variational sample size has a significant impact on the convergence of the ELBO. Using a larger variational sample size significantly reduces the variance of the ELBO and thus its gradient (@fig-elbo-trace3). Furthermore, a stable ELBO plateau is quicker achieved when utilizing larger samples drawn from the variational distribution. This lower variance would allows us in general to use a larger learning rate, which consequently results in a faster convergence of the ELBO. Due to using the same seed all traces converge to a similar local optima.

Employing a larger batch size results in a reduction of variance, as illustrated in @fig-elbo-trace4 of the ELBO traces. However the reduction is rather minor. Be mindful that the variations in batch size result in different iteration counts per epoch for each run. Therefore, we've adapted @fig-elbo-trace4 by showcasing iterations on the x-axis in this context. Surprisingly, employing a larger batch size in this scenario does not lead to faster convergence of the optimization, in contrast to the observations in @fig-elbo-trace3, where a larger variational sample size does contribute to a faster convergence of the ELBO.
